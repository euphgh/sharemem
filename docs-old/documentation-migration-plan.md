...
- [x] 迁移 SystemVerilog/UVM 开发规范。
- [x] 整理并补全阶段 4 已创建的 `verification-status.md`。
- [x] 明确功能完成度由 `plan/testpoints.md` 维护，问题台账保留现有生命周期；当前无设计待确认项。
- [x] 为容易变化的状态记录检查日期和对应源码提交。
- [x] 复核稳定的接口和算法规则已进入 spec/component 文档，status 只保留实现差异和验收方法。

验收条件：状态文档可以独立更新而不扰动设计文档，开发规范不包含过期阶段说明。

状态：已于 2026-08-06 完成。新规范只约束新增或本次实际修改的仓库自有代码，
不要求对 legacy 文件做无关批量格式化，并排除第三方源码。状态台账保留原有
生命周期，新增 `SHMINS-011` 记录无效 transaction compare API；无完整证据的
历史修复不追溯为已完成项。

### 阶段 8：一致性检查和阅读验证

- [x] 检查全部 Markdown 相对链接。
- [x] 检查根 `AGENTS.md` 和源码注释中的文档链接。
- [x] 搜索旧类名、旧 package、旧目录和失效命令。
- [x] 核对文档中的参数、端口、plusarg、UVM error ID 和源码路径。
- [x] 检查同一规则是否在多篇文档中存在两个完整定义。
- [x] 检查旧文档每个有效章节是否已经迁移或明确废弃。
- [x] 使用无上下文阅读方式回答典型开发问题，确认索引能够引导到正确文档。
- [x] 检查文档首段是否自然说明主要内容和必要前置关系。
- [x] 检查文档中没有统一元数据块和上一篇/下一篇导航。

验收条件：所有链接有效，典型任务都有明确阅读入口，没有已知的重复职责或历史
命名残留。

状态：已于 2026-08-06 完成。以提交 `304c232` 和阶段 8 工作树为基线，检查了
`AGENTS.md` 与当前 `docs/` 的 32 个导航/正文文件、212 条相对链接及章节锚点；没有
失效目标。核对中修正了两个章节锚点、TC 的 external busy 概率、DUT 参数表、LST
include 示例和 `VTAB_D` 的旧注释，并确认 reservation 组件文档覆盖源码中的 41 个
report ID。根 TC 可见的 100 个定义与根 regression 的 85 个唯一 case 全部对应，
没有缺失 case。旧文档的有效章节均已由阶段 0 映射到当前正文，obsolete API 方案已
明确废弃；第三方 `sv-collection` 文档不纳入仓库规范迁移。

从无上下文入口分别查找地址映射修改、reservation checker 调试、新增 testcase、
macOS/Ubuntu 检查、已知实现缺口和 SystemVerilog 修改规范时，均可由顶层或 ut_shm
索引进入唯一的详细文档。当前文档没有统一元数据块、文末上一篇/下一篇导航或两个
并行维护的完整协议定义。`docs-old/` 的保留或删除仍按完成标准第 10 项由用户决定。
