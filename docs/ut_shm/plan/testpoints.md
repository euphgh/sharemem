...
P0-3/P0-4 的四个真实 RTL directed test 也已进入根 TC 和独立 LST；新增 test 只有空
design 编译证据，尚未保存真实 RTL 结果和目标 bin 命中证据。其他 testpoint 仍缺少对应
covergroup，因此本文仍没有 testpoint 可标记为“已闭环”。本批暂不要求全项目 coverage
merge 或总百分比阈值。
...
|数据路径|`TP-DATA-001`～`004`|双 gid正向主路径和reference隔离组件通过；gid隔离/M2V边界case已实现，待真实RTL与coverage|
|地址模型|`TP-ADDR-001`～`009`|BLK公式/回归和双gid物理公式组件通过；wpid边界case已实现，待真实RTL与coverage|
|MEM|`TP-MEM-001`～`005`|统一 monitor、gid resolver 和模型正向主路径已通过 109-case 真实 RTL 回归；FFD_CYC 和定向负例未完成|
|Reservation|`TP-RSV-001`～`008`|结构化outcome、第一批coverage和组件测试已接入；ownership case已实现，待真实RTL与coverage|
...
|`TP-MEM-005`|[`VLM-012`](../spec/mem-vlm-interface.md#6-检查规则)：MEM 无 gid，只有唯一到期 reservation match 才能补全 gid并更新数据模型|正常 match、unexpected、missing、地址错误，以及两个 gid相同 BADDR 的隔离访问|resolver 的 gid/gid_valid/matched；read driver和 scoreboard只消费完全匹配事务|direction × gid × match outcome × same-BADDR-different-gid|P0-1 resolver组件矩阵通过；相同BANK/BADDR跨gid真实RTL数据隔离case已实现并编译|正常双gid case源码已具备；待真实RTL/bin证据，resolver负例仍仅有组件证据|
...
|`TP-RSV-007`|[`VLM-010`](../spec/mem-vlm-interface.md#6-检查规则)：other-gid external busy允许，other-gid同bank/due DUT record阻塞|对同一候选交替构造两种busy所有者，并覆盖不同bank对照|scheduler ownership与checker target/pending判定|direction × candidate gid × other-gid source × same/different bank × outcome|P0-1组件矩阵通过；other-gid external允许和target-gid阻塞真实RTL case已实现并编译|external所有权case源码已具备；待真实RTL/bin证据，other-gid SHM冲突仍缺RTL定向|
|`TP-RSV-008`|[`VLM-012`](../spec/mem-vlm-interface.md#6-检查规则)：到期record为MEM恢复唯一gid|两个gid相同地址正常兑现，及unexpected/missing/address mismatch|resolver result、memory transaction metadata和可信模型是否更新|direction × gid × match outcome × address relation|P0-1 resolver/metadata组件矩阵通过；相同BADDR双gid正常兑现的真实RTL case已实现并编译|正常路径case源码已具备；待真实RTL/bin证据，resolver负例仍仅有组件证据|
