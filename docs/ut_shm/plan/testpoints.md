...
|Reservation|`TP-RSV-001`～`006`|checker 已覆盖大部分时序/匹配规则；alignment policy 已移出，reset 和 coverage 有缺口|
...
|`TP-ADDR-007`|[M2V 写回和 beat 对齐](../spec/address-model.md#8-访问来源与-mem-beat-对齐)：m-read 和普通 V2M beat 对齐；M2V v-write、VTRANS write 可非对齐；M2V 写回位于所选 WARP 12 KiB|各来源 aligned/nonaligned 正反例，vaddr 0/最大值，写回跨 beat，普通 V2M element 位于 beat 边界|scoreboard 关联实际 MEM 与原始 creq；reservation 只检查完整地址兑现。Alignment 检查暂缓见 `SCB-001`|source × direction × aligned class × vaddr boundary × beat crossing|V2M/M2V/VTRANS case 都可能随机命中，但没有定向来源分类 case|激励部分、检查缺失、coverage 缺失、case 部分；总体部分实现|
...
|`TP-RSV-005`|[`VLM-003`](../spec/mem-vlm-interface.md#6-检查规则)、`VLM-007`：read 对齐；write 对齐按来源；合法非对齐 write 必须由完全相同的 `mem_waddr` 兑现|普通 V2M、M2V v-write、VTRANS 的 aligned/nonaligned 正反例，并改变低 5 bit|reservation checker 只做完整地址比较；所有 alignment policy 暂缓由 scoreboard 实现，见 `SCB-001`|source × aligned class × address-low-bits × match result|reservation 定向例覆盖非对齐地址原样兑现和低位 mismatch；无来源分类 case|完整地址检查已有、alignment 检查缺失、coverage 缺失、case 部分；总体部分实现|
