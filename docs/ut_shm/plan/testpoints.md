...
|MEM|`TP-MEM-001`～`004`|写比对和固定延迟 read service 已接入，四态和 FFD_CYC 不完整|
|Reservation|`TP-RSV-001`～`006`|checker 已覆盖大部分时序/完整地址匹配规则，reset 和 coverage 有缺口|
...
|`TP-ADDR-007`|[MEM beat 地址与 byte lane](../spec/address-model.md#8-mem-beat-地址与-byte-lane)：read/write beat 均允许非对齐，lane `k` 对应 `beat_addr+k`；M2V 写回仍位于所选 WARP 12 KiB|read/write address 低 5 bit、跨传统 32-Byte 边界、vaddr 0/最大值和 sparse strobe|memory model 按完整地址返回连续 32 Byte；scoreboard 展开 strobe 检查逐 byte 地址和数据；reservation 完整地址兑现|direction × address-low-bits × boundary crossing × strobe pattern|V2M/M2V/VTRANS case 可能随机命中；reservation 定向例覆盖非对齐完整地址|激励部分、检查部分、coverage 缺失、case 部分；总体部分实现|
...
|ID|Spec 与验证目标|所需激励|观察点与 checker|目标 coverage|当前 case|状态与缺口|
|---|---|---|---|---|---|---|
|`TP-MEM-001`|[`MEM-002`](../spec/mem-vlm-interface.md#6-检查规则)、`MEM-003`：有效 read/write 的完整地址已知；write strobe 和有效 data lane 已知且 strobe 非零|read/write 分别注入 valid/address/strobe/data X/Z 和零 strobe；valid 0 时 payload 任意|memory monitor 和 reservation monitor；当前只覆盖部分 valid/address，完整四态缺口见 `VMEM-002`|direction × field × known/XZ × valid|普通 V2M/M2V case 产生合法流量；无负向 case|激励缺失、检查部分、coverage 缺失、case 缺失；总体部分实现|
|`TP-MEM-002`|[`MEM-003`](../spec/mem-vlm-interface.md#6-检查规则)和 [写事务](../spec/mem-vlm-interface.md#22-写事务)：strobe 为 1 的 lane 写入 `mem_waddr+lane`，0 lane保持；地址允许非对齐|full/sparse/single-lane strobe、非零低 5 bit、跨传统 32-Byte 边界和同 bank 多周期写|memory monitor transaction、`rtl_banks` 更新、reference byte map 和 scoreboard|source × strobe pattern × DTYPE × address-low-bits × boundary crossing|V2M/M2V/VTRANS 提供宽泛数据流，无 strobe/低地址位定向 case|激励部分、检查部分、coverage 缺失、case 部分；总体部分实现|
