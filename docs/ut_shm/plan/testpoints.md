...
|`TP-DATA-004`|[架构顺序](../environment/components/shm-scoreboard.md#4-实际-write-比对)：DUT 可乱序兑现重叠 creq，但最终 memory 必须等价于 creq 顺序执行|定向构造完全重叠、部分重叠、链式覆盖和非重叠 creq，并控制 DUT 先兑现旧值或最终值|scoreboard `wmap_final/wmap_expired`、record matched/expired/unmatched，以及 drain 后 `ref_banks/rtl_banks` touched-byte 最终比较|overlap class × observed order × final/expired match × final convergence|已实现 `ORDER-M-001`～`003`、`ORDER-V-001` 四个最小背靠背 case；hazard、queue 和 final-memory 组件矩阵通过，正式 RTL 尚待运行|最小定向激励、batch checker 和最终比较已实现；RTL 结果、扩展矩阵及 coverage 尚缺，总体实现中|
