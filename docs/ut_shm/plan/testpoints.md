...
|ID|Spec 与验证目标|所需激励|观察点与 checker|目标 coverage|当前 case|状态与缺口|
|---|---|---|---|---|---|---|
|`TP-ADDR-001`|[Offset 解码](../spec/address-model.md#32-offset-解码)：ATYPE_W 32/16/8、signed/unsigned、1B/DW granularity 与 DTYPE 共同得到 `E[t][k]`，禁止截断回绕|覆盖正/负/零/边界 raw offset，所有 width/sign/granularity/DTYPE 组合|monitor 解码字段、reference MADDR 和最终 BANK/BADDR/数据；需要独立公式级 reference 测试|ATYPE_W × sign × granularity × DTYPE × offset sign/boundary|叶子 case 只有 ATYP_32/16；S/G 无有效约束，见 `SHMINS-004`；ATYP_8 无 case|激励部分、检查部分、coverage 缺失、case 部分；总体部分实现|
...
- V2M/M2V 矩阵能固定 direction、ITYPE、SPACE、DTYPE 和 ATYPE_W，但没有 functional
  ...
  reservation 场景已经出现。
- VTRANS 只有一个 case，DTYPE 与 ITYPE 组合由 seed 决定，且 `creq_tmsk` 未贯通、未约束全 1。
