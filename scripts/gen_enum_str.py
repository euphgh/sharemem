...
def generate_functions_only(
    enums, case_insensitive=True, strip_prefix=False, include_guard=None
):
    ...
    if include_guard:
        lines.append("`endif // %s" % include_guard)

    return "\n".join(lines)


def output_include_guard(output_path):
    """Return the canonical include guard for a generated .svh output."""
    if not output_path or Path(output_path).suffix.lower() != ".svh":
        return None

    normalized_name = re.sub(r"[^A-Za-z0-9]+", "_", Path(output_path).name)
    return "INC_%s" % normalized_name.upper().strip("_")
...
def main():
    ...
    if args.no_pkg:
        output = generate_functions_only(
            enums,
            case_insensitive,
            strip_prefix,
            include_guard=output_include_guard(args.output),
        )
    else:
