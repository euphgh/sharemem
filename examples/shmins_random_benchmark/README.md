...
|`BENCH_PROFILE`|`RANDOM`|随机分支或固定 instruction/space profile|
...
固定 profile 都使用 `DTYP_32 + ATYP_32 + ATYP_U + GAUTO_1B`，避免 dtype/atype 分支
变化干扰对比。可选值为：
...
- `NO_UNIQUENESS`：只关闭 `c_addr_offs_elem_ne`；
- `CORE`：同时关闭上述两组。

后三种只用于判断时间消耗来自哪个约束组，产生的 item 不代表合法 DUT 激励。
如果需要观察关闭约束后产生的越界地址，可以设置 `BENCH_SUPPRESS_ITEM_WARNINGS=0`；
这种运行的耗时不能再和正常基准直接比较。

例如固定 seed 测量 `LDSTE_V + SPACE_WRP`：

```sh
scripts/ubuntu/run_shmins_random_benchmark.sh run \
  +BENCH_PROFILE=LDSTE_V_WRP \
  +BENCH_ITERATIONS=100 \
  +BENCH_WARMUP=5 \
  +ntb_random_seed=12345
```

隔离全局 uniqueness/non-overlap 约束的成本：

```sh
scripts/ubuntu/run_shmins_random_benchmark.sh run \
  +BENCH_PROFILE=LDSTE_V_WRP \
  +BENCH_CONSTRAINT_SET=NO_UNIQUENESS \
  +BENCH_ITERATIONS=100 \
  +ntb_random_seed=12345
```
...
- `elapsed_ns`：正式 randomize 循环的单调时钟耗时；
- `ns_per_attempt` 和 `attempts_per_second`：主要性能指标；
