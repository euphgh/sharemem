...
module alignment_tb;
  ...
  initial begin
    ...
    // The exact-address negative case is checked through result counters instead of UVM report output.
    ...
    // Write port 1 also accepts and preserves a non-32-byte-aligned reservation.
    bank = 3;
    ...
    result = reservation_checker.check_cycle(txn);
    check_true(result.passed, "write port 1 nonaligned reservation was rejected by checker");
    scheduler.process_cycle(txn);
    check_true(scheduler.shm_records[VLM_RESERVATION_WRITE][1][bank] != null,
               "write port 1 nonaligned reservation was not scheduled");
    check_true(scheduler.shm_records[VLM_RESERVATION_WRITE][1][bank].address == 'h123,
               "scheduler changed write port 1 address low bits");
    check_true(scheduler.shm_records[VLM_RESERVATION_WRITE][1][bank].write_port == 1,
               "scheduler did not retain write port 1 provenance");

    initialize_transaction(txn, 5);
    result = reservation_checker.check_cycle(txn);
    check_true(result.passed, "write port 1 record failed while advancing to its due cycle");
    scheduler.process_cycle(txn);
    ...
    result = reservation_checker.check_cycle(txn);
    check_true(result.mem_match_error_count == 0 && result.matched_mem_request_count == 1,
               "exact nonaligned MEM write from port 1 did not match its reservation");
    scheduler.process_cycle(txn);

    // Issue another nonaligned write reservation for an exact-address mismatch test.
    bank = 4;
    ...
    txn.rsv_wreq_array[bank][0].delay   = 2;
    result = reservation_checker.check_cycle(txn);
    check_true(result.passed, "write reservation for exact-address mismatch test was rejected");
    scheduler.process_cycle(txn);

    initialize_transaction(txn, 8);
    result = reservation_checker.check_cycle(txn);
    check_true(result.passed, "write record failed before its exact-address mismatch cycle");
    scheduler.process_cycle(txn);

    // An aligned address in the same 32-byte beat is not an exact match for 0x123.
    initialize_transaction(txn, 9);
    txn.mem_wreq_array[bank] = new();
    ...
    // Read reservations are also scheduled without alignment policy; exact address matching remains mandatory.
    bank = 5;
    initialize_transaction(txn, 10);
    txn.rsv_rreq_array[bank] = new();
    txn.rsv_rreq_array[bank].address = 'h145;
    txn.rsv_rreq_array[bank].delay   = 2;
    result = reservation_checker.check_cycle(txn);
    check_true(result.passed, "nonaligned read reservation was rejected by checker");
    scheduler.process_cycle(txn);
    check_true(scheduler.shm_records[VLM_RESERVATION_READ][1][bank] != null,
               "nonaligned read reservation was not scheduled");
    check_true(scheduler.shm_records[VLM_RESERVATION_READ][1][bank].address == 'h145,
               "scheduler changed read reservation address low bits");

    initialize_transaction(txn, 11);
    result = reservation_checker.check_cycle(txn);
    check_true(result.passed, "read record failed while advancing to its due cycle");
    scheduler.process_cycle(txn);

    initialize_transaction(txn, 12);
    txn.mem_rreq_array[bank] = new();
    txn.mem_rreq_array[bank].address = 'h145;
    result = reservation_checker.check_cycle(txn);
    check_true(result.mem_match_error_count == 0 && result.matched_mem_request_count == 1,
               "exact nonaligned MEM read address did not match its reservation");
    scheduler.process_cycle(txn);

    $display("vlm reservation alignment-ownership regression: PASS");
    $finish;
  end
