...
import shm_util_package::GID_N;
import shm_util_package::PHYSICAL_BANK_N;
import shm_util_package::BADDR_W;
import shm_util_package::physical_bank_index;
import collection::aa_array_util;
import collection::aa_util;

class vlm2aa;
    typedef bit [BADDR_W-1:0] baddr_t;
    typedef aa_array_util#(PHYSICAL_BANK_N, baddr_t, byte) wmap_util;
    // Flattened <logical bank, gid> write trace. Use physical_bank_index() to form the first index.
    ...
    static function bit trans(vlm_memory_sequence_item vlm, ref wmap_t res);
        ...
        for (int bk_id = 0; bk_id < BANK_N; bk_id ++) begin: bank_loop
            ...
            if (!vlm.gid_valid[bk_id] || !vlm.reservation_matched[bk_id]) begin
                continue;
            end
            physical_bank = physical_bank_index(bk_id, int'(vlm.vlm_gid[bk_id]));
